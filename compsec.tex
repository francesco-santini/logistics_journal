...
%\affiliation{organization={Dipartimento di Matematica e Informatica, Universit{\`a} degli Studi di Perugia}, addressline={Via Vanvitelli 1},  city={Perugia}, postcode={06123},  country={Italy}
%}
...
\begin{highlights}
	\item Intelligent homes and buildings are equipped with different devices by different vendors.
	\item  These devices are guided by a variety of protocols and frameworks, such as IoT, ICS, and off-the-shelf virtualised containers.
	\item  We log attacks towards such targets with the purpose to estimate how much we are safe at home.
	\item Honeypots are adopted to study the behaviour of attackers.
\end{highlights}
...
\begin{keyword}
Honeypot, ICS, S7comm, Modbus, medium interaction.
\end{keyword}
...
\emph{Industrial Control Systems} (\emph{ICS})~\cite{ICS} are command and control networks and systems designed to support industrial processes. These systems also operate critical infrastructures, such as electricity generation plants, transportation systems, oil refineries, chemical factories and manufacturing facilities: hence, protecting the is crucial for the safety of citizens.
\emph{Operational Technology} (\emph{OT}) is a term that collectively concern hardware and software that detects or causes a change through the direct monitoring and/or control of physical devices, processes and events in such systems. Examples are \emph{Supervisory Control and Data Acquisition} (\emph{SCADA}) and \emph{Programming Logic Controllers} (\emph{PLC}). 

Initially, ICS were isolated by nature~\cite{ICS}, being limited to the process network.
...
However, nowadays, in the era of Industry 4.0 and IoT, ICS are able to be interconnected.
This need for connection, also combining the questionable paradigm of making
open standards, clearly exposes ICS to serious safety problems.

We can think about resorting to protection measures adopted in conventional ICT systems, such as a Firewall or an IDS.
...
on the other hand, ICS are designed to ensure \emph{reliability/availability}, even jeopardizing the \emph{confidentiality} and \emph{integrity} of data. Further common goals in ICS are related to the \emph{safety} of operators and environment, and, of course, \emph{productivity}.
...
In this paper we present \emph{LOGistICS}, an emulation and monitoring system for OT. In particular, we focus on two typical ICS protocols, such as S7comm from Siemens~\cite{xiao2017s7commtrace} and Modbus from Schneider Electric~\cite{modbustcp}, 
which are also used extensively in home and building automation.
The framework is composed of \emph{i)} a highly customizable \emph{honeypot} that emulates the two aforementioned ICS services,
\emph{ii)} a sniffer that records traffic, and \emph{iii)} a monitoring node, which is capable of analyzing and plotting recorded data.
...
The proposed system is light, stealthy, isolated through containerization technologies, cross-platform, easy to deploy, with a fair degree of interaction.
Our honeypots are based on two libraries that allow communication via S7comm and Modbus protocols: \emph{Snap7}\footnote{Snap7: \url{http://snap7.sourceforge.net}.} and \emph{Pymodbus}\footnote{Pymodbus: \url{https://pymodbus.readthedocs.io/en/latest/}.}.
With the purpose to deceive search engines, we made some low-level changes to make it more faithful to a real PLC.
...
What proposed in this paper goes under the umbrella of \emph{deception technology}, which is an emerging category of cybersecurity defense. Deception technology automates the creation of traps (decoys), and can detect, analyze, and defend against zero-day and advanced attacks, often in real time.
...
Most of the honeypots described in Sect.~\ref{sect:related} offer a low level of interaction.
...
To conclude this section, we summarize the six main design principles we want to adopt in order to assemble our ICS honeypot:
\begin{itemize}
...
Among all the components of LOGistICS, this paper focuses on the implementation of a honeypot based on Modbus (502/tcp) and S7comm (102/tcp). The remainder of the paper is organized as follows: Sect.~\ref{sect:related} summarizes the related work in the literature and gave a minimal background to the reader. Sect.~\ref{sec:implementation} describes the implementation of the systems, while Sect.~\ref{sect:experiments} reports some of the tests we performed to evaluate LOGistICS: for example how much the platform can deceive reconnaissance tools. Finally, in Sect.~\ref{sect:conclusion} we conclude the paper with final thoughts and several ideas about possible future work. 

\section{Related Work}\label{sect:related}
In this section we collect some of the most important works and implementations that concern ICS honeypots.
...
Moreover, Conpot provides templates written in \emph{xml}, in order to extend/modify the fingerprint of each emulated service.

Mirian et al. deployed some Conpot instances and a network telescope~\cite{mirian2016internet} (i.e. a cluster of routed IP addresses space in which most of the traffic is suspicious).
in order to monitor the scanning traffic concerning ICS devices.
The work in \cite{zanero} is a more recent study that extends the Mirian et al. work. The authors monitored the scanning traffic deploying Conpot instances on cloud hosting platforms. 
Mirian et al. used the default Conpot template, whereas in this paper the authors used different Conpot templates.

%Edit
In \cite{serbanescu2015ics} the authors used a honeynet of low-interaction ICS honeypots: a honeynet is a collection of honeypots in a controlled and monitored network. 
The honeynet was deployed on Amazon EC2 Cloud\footnote{Amazon EC2 Cloud:~\url{https://aws.amazon.com/ec2/}.} 
...
The authors stated that if there are several protocols exposed, one can affect the traffic of the other (e.g., SNMP might favor interactions with DNP3). 
The authors also noted that if a honeypot is indexed on Shodan, its number of interactions increases.
...
and it implements the capture of \emph{Ladder Logic} programs: Ladder Logic Programming is a graphical PLC programming language which expresses logic operations using a symbolic notation.
...
It also implemented an extended version of Whaler~\cite{whaler}, a honeypot that emulates Docker~\cite{merkel2014docker} API allows to remotely manage containers.
This study shows that anomalous traffic to the ports used by smart-home devices is mostly harmful. 
%The study was extended by including Conpot, and comparing the results with this honeypot with those previously collected. Unlike the study conducted on attacks on smart-home devices, for ICS services most the anomalous traffic is due to public scanners (e.g., Shodan).\footnote{The inclusion of Docker in the world of home/building-automation is justified by the fact that 	Microsoft IoT Edge OPC UA and Eclipse Mosquitto (i.e., an MQTT broker) have been downloaded more than 10M and 100k times respectively (source: \url{https://hub.docker.com/}).}
...
In Sect.~\ref{sec:architecture} we describe the overall experimental setup in its entirety.
Sect.~\ref{sec:monitoring_node} identifies the components of the monitoring node.
In Sect.~\ref{sec:packet_capture} we describe the sniffer we implemented. 
We mainly focus on its versatility, and on its ability to listen only to the traffic of interest. 
In Sect.~\ref{sec:modbus_service} we describe the implementation of the Modbus service based on Pymodbus, by also explaining how we enriched the functionality of the emulated service.
We implemented a novel feature with respect to the honeypots in the literature (see Sect.~\ref{sect:related}): 
the creation of customizable and easily extensible templates using a csv file. 
In these different templates we populated hardware-version fields by selecting existing PLC that support the Modbus/TCP standard: other proposals in the literature (see Sect.~\ref{sect:related}) do not allow such extensibility.
In Sect.~\ref{sec:s7comm_service} we describe the implementation of Snap7 based on S7comm service.
...
\
The framework, whose architecture is represented in Fig.~\ref{fig:logistics_architecture}, can be summarized by four main components:
\begin{itemize}
...
The attack perimeter unfolds over LANs and WANs.
...
infrastructure.
The Honeypot node is a second Raspberry Pi that exposes Modbus and S7comm services. In order to isolate and
to ensure portability, we distribute these services via Docker.
The sniffing node is a third Raspberry Pi on which we launched an instance of our sniffer.
...
The monitoring node is a \emph{Mini PC}\footnote{AMD Ryzen 5, 2.1GHZ Quad Core with 16Gb of RAM.} that contains an \emph{ELK stack}\footnote{https://www.elastic.co/what-is/elk-stack.} instance, which we describe in Sect.~\ref{sec:monitoring_node}.
...
The honeypot and monitoring nodes implement \emph{Docker}\footnote{Docker: \url{https://www.docker.com}.} as a virtualization technology.
...
\emph{Logstash} and \emph{Kibana} collectively known as the \emph{ELK} stack. In this paper we use ELK mostly to manipulate and query log files in order to provide an offline investigation of what collected during the exposition of the proposed honeypot to the Internet. However, in the future we plan to extend the monitoring component with the purpose to leverage ELK as a \emph{Security Information and Event Management} (\emph{SIEM}) (see Sect.~\ref{sect:conclusion}).

\subsubsection{{\bf Logstash}}\label{subsec:logstash}
Logstash allows for collecting data from multiple systems, where data can
...
We use Logstash both to enrich the logs (e.g., with DNS-PTR that resolves an IP address to a domain/host name, or Geolocation), and to filter useless data out.



\subsubsection{{\bf Elasticsearch}} \label{subsec:elasticsearch}
Elasticsearch is a full text search engine based on \emph{Lucene},\footnote{Lucene: https://lucene.apache.org/.} a project
...
\emph{create}, \emph{read}, \emph{update}, and delete are the four basic functions of persistent storage. We used Elastisearch to group logged activities into meaningful time series.

\subsubsection{{\bf Kibana.}}\label{subsec:kibana}
Kibana is a dashboard designed to display data stored on Elasticsearch. It
...
interface enables a user to quickly create and share dynamic dashboards. In this first implementation, Kibana is the only ELK tool we did not directly use: the following graphs are generated with Python in the monitoring node. However, it will be adopted when ELK will be used as a SIEM. 
...
We need to give the user the ability to view the interface before starting the capture. 
...
As shown in List.~\ref{lst:argparse_sniffer}: we need to pass the network interface as a required argument.
...
We prefilter\footnote{Berkley Packer Filter (\emph{BPF}) syntax -~\url{https://biot.com/capstats/bpf.html}.} the traffic by only considering the ports of interest of the honeypot, thus reducing the size of a pcap file.
...
An output example of the implemented procedure is shown in List.~\ref{lst:debug_sniffer}.
...
We implemented two asynchronous functions: the first one updates the slave values, and the other returns the values of the slave to the screen (We use the stream handler
...
\begin{table}[t]
	...
	\begin{adjustbox}{width=0.46\textwidth}
		...
		\end{tabular}
	\end{adjustbox}
	\caption{Available default Modbus-PLC templates.}
	\label{tab:modbus_templates}
\end{table}


We guarantee an easy customization of templates in csv format, making it usable also through \emph{Office Business Applications} (\emph{OBA}).
An user can enter parameters such as the IP address directly from command line, interface and PLC template. In List.~\ref{lst:modbus_argparse} we describe the honeypot configuration parameters.
...
In Fig.~\ref{lst:hexdump_szl001c} we show (an excerpt of) the reverse hexdump of this field. By using \emph{-r -p} flags, we are able to read hexadecimal dumps as text.

\begin{lstlisting}[basicstyle=\small, label={lst:hexdump_szl001c},float=htb,caption={An excerpt of Reverse Hexdump on SZL ID 001c field
...
capable of selecting the class of Siemens PLC to be emulated, and quickly start it (see Fig.~\ref{fig:plc_choose}).
...
In the aforementioned Siemens PLCs, as per default configuration data-blocks are not protected and can be read and modified by users~\cite{rogue7}. To test the degree of interaction before the exposure phase to the Internet (see Sect.~\ref{sect:experiments}), we used a C++ master that executed requests to the considered PLCs.
...
In order to allow a bidirectional exchange of ICS traffic, we use \emph{nftables} (a successor of iptables). 
...
We divide this section into three distinct parts. First, Sect.~\ref{sec:reconnaissance} shows how much the honeypot we implemented is recognizable by specific detection-tools. Then Sect.~\ref{sect:pentest} shows how our honeypot reacts to well-known PLC exploits. Finally, Sect.~\ref{sec:logging} presents the result by leaving the honeypot logging traffic from the Internet.
...
In List.~\ref{lst:nmap_mdbs} and in List.~\ref{lst:s7_template}, 
...
We executed these two tools on our honeypot with success, obtaining results similar to those obtained with NSE scripts. Listing~\ref{lst:plcscan_beckhoff} and List.~\ref{lst:plcscan_1200} show how much comprehensive the answer obtained by PLCScan is, for both Modbus and S7comm.
...
Then we executed Honeyscore on our instance having the emulated Modbus service. The score, between $0$ and $1$ represents the probability of having scanned a honeypot instead of a real PLC. As shown in List.~\ref{lst:honeyscore_setup}, our system with the Modbus service enabled is classified as real.
...
\begin{table}[htb]
	...
	\end{tabular}
	\caption{A comparison of LOGistICS and Conpot default templates by using Honeyscore.}
	\label{tab:honeyscore_tests}
\end{table}

In Tab.~\ref{tab:honeyscore_tests} we represent the results conducted by using Honeyscore. 
...
In Tab.~\ref{tab:honeyscore_tests} we also show the scores obtained by testing Conpot, which appears as always having a definitely higher probability of being detected when some port is active.
...
In Tab.~\ref{tab:ics_exploitdb} we collected some exploits that are compatible with our services; in this table, 
...
As
an example we launch the ISF script in Tab.~\ref{tab:ics_exploitdb} targeting a Modicon PLC. As we can see in 
List.~\ref{lst:metasploit_modbus}, the service was active during the exploitation phase and no errors occurred: the
...
Just to report here an example, Tab.~\ref{tab:ics_exploitdb} shows the result obtained by launching the last exploit in Tab.~\ref{tab:ics_exploitdb} by using ISF. As we can see in List.~\ref{lst:metasploit_modbus}, the service was active during the exploitation phase no errors occurred.
The CPU was successfully compromised according to ISF. We omit the output of the exploit towards the S7comm protocol as it is identical to List.~\ref{lst:metasploit_modbus}.
...
In Fig.~\ref{fig:evo_ics} we notice that on some days there is a positive correlation between the events of the two services, while in some other days there is not.
...
Fig.~\ref{fig:modbus_fc} shows the number and type of Modbus requests we logged in one month.
...
This type of request, as we also see in the Tab.~\ref{tab:modbus_actors}, was always performed by scanners and we believe it is therefore a benign request.
We logged unauthorized reading requests to the \emph{holding registers}, which basically store 16 bits readable and writable configuration-values. 
...
Fig.~\ref{fig:s7comm_fc} concerns S7comm events instead. We collected a large number of \emph{Setup Communication requests}, used to establish a communication with the S7comm Layer. 
...
Given the enormous amount of source hosts, Tab.~\ref{tab:modbus_actors} and Tab.~\ref{tab:s7comm_actors}
...
Only after this stage did it perform malicious activities such as attempting to tamper with the CPU as shown in Tab.~\ref{tab:s7comm_actors}. 
...
\begin{remark}\label{rmk:syn}
	In Fig.~\ref{fig:s7comm_fc} we report all the activities of the S7comm application layer.
...
From what accomplished we collected several ideas about possible extensions as future work. For instance, we would like to ELK as a SIEM.
...
%Finally, we will group operations requested by those masters probing our honeypot: the aim is to 
